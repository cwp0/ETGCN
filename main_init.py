...
DATA_PATHS = {
    ...
    "Melbourne": {"feat": "data/eua_loads_1.txt", "adj": "data/eua_adj_1.csv"},
}
...
def get_model(args, dm):
    ...
    if args.model_name == "GRU":
        model = models.GRU(input_dim=dm.adj.shape[0], num_features=args.num_features, hidden_dim=args.hidden_dim)
    if args.model_name == "TGCN":
        model = models.TGCN(adj=dm.adj, hidden_dim=args.hidden_dim)
    if args.model_name == "ETGCN":
        model = models.ETGCN(adj=dm.adj, num_features=args.num_features, hidden_dim=args.hidden_dim)
    return model


def get_task(args, model, dm):
    task = getattr(tasks, args.settings.capitalize() + "ForecastTaskWithGraph")(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, help="The name of the dataset", choices=("shenzhen", "losloop", "Melbourne"), default="losloop"
    )
    parser.add_argument(
        "--model_name",
        type=str,
        help="The name of the model for spatiotemporal prediction",
        choices=("GCN", "GRU", "TGCN", "ETGCN"),
        default="GCN",
    )
    parser.add_argument(
        "--settings",
    ...
    parser.add_argument("--log_path", type=str, default="./result.log", help="Path to the output console log file")
