...
class GRULinear(nn.Module):
    def __init__(self, num_gru_units: int, num_features: int, output_dim: int, bias: float = 0.0):
    ...
    def forward(self, inputs, hidden_state):
        ...
        # [inputs, hidden_state] "[x, h]" (batch_size, num_nodes, num_gru_units + num_features)
        concatenation = torch.cat((inputs, hidden_state), dim=2)
        # [x, h] (batch_size * num_nodes, gru_units + num_features)
        concatenation = concatenation.reshape((-1, self._num_gru_units + num_features))
...
class GRUCell(nn.Module):
    def __init__(self, input_dim: int, num_features: int, hidden_dim: int):
        ...
        self.linear1 = GRULinear(self._hidden_dim, self._num_features, self._hidden_dim * 2, bias=1.0)
        self.linear2 = GRULinear(self._hidden_dim, self._num_features, self._hidden_dim)
...
class GRU(nn.Module):
    def __init__(self, input_dim: int, num_features, hidden_dim: int, **kwargs):
        super(GRU, self).__init__()
        self._input_dim = input_dim  # num_nodes for prediction
        self._num_features = num_features
        self._hidden_dim = hidden_dim
        self.gru_cell = GRUCell(self._input_dim, self._num_features, self._hidden_dim)
